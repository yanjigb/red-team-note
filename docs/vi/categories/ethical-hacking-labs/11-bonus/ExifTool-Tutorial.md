...
ExifTool là một thư viện Perl đa nền tảng kết hợp ứng dụng dòng lệnh dùng để **đọc, ghi và chỉnh sửa siêu dữ liệu trong nhiều loại tệp khác nhau**.

ExifTool hỗ trợ nhiều định dạng siêu dữ liệu khác nhau bao gồm EXIF, GPS, IPTC, XMP, JFIF, GeoTIFF, ICC Profile, Photoshop IRB, FlashPix, AFCP và ID3, Lyrics3, cũng như maker notes của nhiều máy ảnh kỹ thuật số từ Canon, Casio, DJI, FLIR, FujiFilm, GE, GoPro, HP, JVC/Victor, Kodak, Leaf, Minolta/Konica-Minolta, Motorola, Nikon, Nintendo, Olympus/Epson, Panasonic/Leica, Pentax/Asahi, Phase One, Reconyx, Ricoh, Samsung, Sanyo, Sigma/Foveon và Sony. (...)

1. [Trang web chính thức](https://exiftool.org/)
2. [Hướng dẫn cài đặt](https://exiftool.org/install.html)

### Mục tiêu:
* Sử dụng exifTool
* Trích xuất siêu dữ liệu từ bất kỳ tệp nào
...
### 2. Trích xuất siêu dữ liệu từ hình ảnh
```bash
