...
0. Use `ifconfig` to see your current MAC address of your Network adapter:
`ifconfig`

1. Turn off the Network adapter:
`ifconfig wlan1 down`

2. Next, change your MAC address to a new random MAC Address using `macchanger`:
`macchanger -r wlan1`
...
0. Turn off the Network adapter:
`ifconfig wlan0 down`

1. Change the address using `hw ether` option from **ifconfig** using any MAC address you want:
`ifconfig wlan0 hw ether 00:11:22:33:44:55`

2. Enable the interface:
`ifconfig wlan0 up`

3. Check the changes of the network adapter:
`ifconfig`
